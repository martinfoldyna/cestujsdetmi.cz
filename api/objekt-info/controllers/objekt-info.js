...
module.exports = {
  ...
  async importOldDb(ctx) {
    ...
    // FIRST: -1 -> 50
      const posts = await db.collection("new_new").find({"id": {$gte: parseInt(start), $lte: parseInt(end)}}).toArray();

    // const post = await db.collection("new_new").findOne({"id": 1});
    const objekty = await Promise.all(posts.map(post => new Promise(async (resolve, reject) => {
      try {

        const alreadyExists = await strapi.services["objekt-info"].findOne({hodnota: post.hodnota})
        ...
        if (!alreadyExists) {
          ...
          let foundMesto = post.mesto_id
            ...
            : null;
          let foundKraj = post.kraj_id ? await strapi.services["kraj"].findOne({old_id: post.kraj_id}) : null;
          let foundOblast = post.oblast_id ? await strapi.services["oblast"].findOne({old_id: post.oblast_id}) : null;

          let mesto = foundMesto ? foundMesto._id : null;
          let kraj = foundKraj ? foundKraj._id : null;
          let oblast = foundOblast ? foundOblast._id : null;
          const provozni_doba = (post.provozni_doba && Array.isArray(post.provozni_doba)) ? post.provozni_doba.filter(item => item !== null ) : []
          let slevy = Array.isArray(post.slevy) ? post.slevy.filter(item => item !== null ) : []
          let ceny = Array.isArray(post.ceny) ? post.ceny.filter(item => item !== null ) : []
          let statistiky = [];
          for (let key in post) {
          ...
          const created = await strapi.services["objekt-info"].create({
            ...post,
            ...{
              created_at: post.createdAt === "0000-00-00 00:00:00" ? new Date(post.first_activation) : new Date(post.createdAt),
              updated_at: post.createdAt === "0000-00-00 00:00:00" ? new Date(post.first_activation) : new Date(post.createdAt),
              provozni_doba,
              statistiky,
              mesto,
              oblast,
